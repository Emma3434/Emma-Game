    ...
    fetch(url, requestOptions)
        ...
        .then(result => {
            ...
            for (i = 0 ; i < result.discussion[0].comments.length ; i++){
                $('<li class="list-group-item align-items-xl-start comment"><img class="profile-chat" src="../image/default.jpg"><div style="float: left"><span class="text-muted d-block">'+result.discussion[0].comments[i].username+'</span><span class="border rounded border-primary shadow-sm d-block message">'+ result.discussion[0].comments[i].message +'</span><span class="text-muted d-block">'+result.discussion[0].comments[i].time+'</span></div></li>').appendTo($('#discussion'));
            }
        })
